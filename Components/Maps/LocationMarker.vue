<script setup lang="ts">
import { ref } from 'vue'
import { defineMapboxMarker } from '#imports'

interface Props {
 markerId: string,
  point: PlacePoint,
  active: boolean
}

interface Emits {
  (event: 'mouseenter'): void
  (event: 'mouseleave'): void
}

const { markerId, point, active } = defineProps<Props>()
defineEmits<Emits>()
const markerRef = ref<HTMLElement | null>(null)

defineMapboxMarker(markerId, { lnglat: point, anchor: 'bottom' }, markerRef, (m) => {
  m.setLngLat(point)
})
</script>

<template>
  <div
    ref="markerRef"
    class="block cursor-pointer p-0"
    @mouseenter="$emit('mouseenter')"
    @mouseleave="$emit('mouseleave')"
  >
    <svg
      class="fill-white"
      :class="active ? 'w-20 h-20 stroke-accent-500' : 'w-16 h-16 stroke-primary-500'"

      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
    >
      <path fill-rule="evenodd" clip-rule="evenodd" d="M11.54 22.351l.07.04.028.016a.76.76 0 00.723 0l.028-.015.071-.041a16.975 16.975 0 001.144-.742 19.58 19.58 0 002.683-2.282c1.944-1.99 3.963-4.98 3.963-8.827a8.25 8.25 0 00-16.5 0c0 3.846 2.02 6.837 3.963 8.827a19.58 19.58 0 002.682 2.282 16.975 16.975 0 001.145.742zM12 13.5a3 3 0 100-6 3 3 0 000 6z" />
    </svg>
  </div>
</template>

<style scoped>

</style>
